...
void drawBtn(int16_t x,int16_t y,int16_t w,int16_t h, uint16_t bg,uint16_t fg,const char* lbl,uint8_t sz=1,uint8_t r=6) {
    ...
    if (sz == 3) { tft.setFont(&CutePixel48pt); tft.setTextSize(1); }
    else if (sz == 2) { tft.setFont(&CutePixel36pt); tft.setTextSize(1); }
...
void drawField(int16_t x,int16_t y,int16_t w,int16_t h, const String &text,bool active,const char* ph,bool masked=false) {
    ...
    tft.setFont(&CutePixel24pt); tft.setTextSize(1);
    
    int16_t x1, y1; uint16_t tw, th;
    tft.getTextBounds("A", 0, 0, &x1, &y1, &tw, &th);
...
void drawKeyboard() {
    ...
    for (int i=0;i<10;i++) {
        ...
        drawBtn(r0x+i*(KW+KGAP),KB_Y,KW,KH,C_KEY_BG,C_WHITE,l,2,4);
    }
    // ROW 1
    ...
    for (int i=0;i<r1l;i++) {
        ...
        drawBtn(r1x+i*(KW+KGAP),r1y,KW,KH,C_KEY_BG,C_WHITE,l,2,4);
    }
    // ROW 2
    ...
    drawBtn(r2x,r2y,shW,KH,(kbPage==1)?C_ACCENT:C_KEY_ACT,C_WHITE,(kbPage==1)?"^A":"^a",2,4);
    for (int i=0;i<r2l;i++) {
        char l[2]={KB_ROW2[kbPage][i],0};
        drawBtn(r2x+shW+KGAP+i*(KW+KGAP),r2y,KW,KH,C_KEY_BG,C_WHITE,l,2,4);
    }
    drawBtn(r2x+shW+KGAP+r2l*(KW+KGAP),r2y,bsW,KH,C_KEY_ACT,C_WHITE,"<=",2,4);
    // ROW 3
    int r3y=KB_Y+3*(KH+KGAP), fnW=80, okW=80, r3x=8;
    int spW=480-(r3x*2)-fnW-KGAP-KGAP-okW;
    drawBtn(r3x,              r3y,fnW,KH,C_KEY_ACT,C_WHITE,kbPage<2?"123":"ABC",2,4);
    drawBtn(r3x+fnW+KGAP,     r3y,spW,KH,C_KEY_SP, C_WHITE,"SPASI",2,4);
    drawBtn(r3x+fnW+KGAP+spW+KGAP,r3y,okW,KH,C_SUCCESS,C_WHITE,"OK",2,6);
}
...
void drawCenteredText(const String &txt, int16_t bx, int16_t by, int16_t bw, int16_t bh, uint16_t color, bool useLargeFont) {
    ...
    if (useLargeFont) {
        if (txt.length() <= 2) {
            tft.setFont(&CutePixel72pt);
        } else if (txt.length() <= 8) {
            tft.setFont(&CutePixel48pt);
        } else {
            tft.setFont(&CutePixel36pt);
        }
        tft.setTextSize(1);
    } else {
        tft.setFont(&CutePixel24pt);
        tft.setTextSize(1);
    }
...
void drawInputCardOnly() {
    ...
    tft.setCursor(258, 114); tft.print("INPUT PENGGUNA");
    ...
    if (currentMode == 3) {
        ...
        tft.setCursor(258, 190); tft.print("Ekspresi kalkulator");
    } else if (currentMode == 2) {
        String val = currentWord.length() ? currentWord : "-";
        drawCenteredText(val, 250, 112, 220, 48, C_WHITE, true);
        tft.setFont(&CutePixel24pt); tft.setTextSize(1);
        tft.setTextColor(C_DGRAY);
        tft.setCursor(258, 190); tft.print("Sedang mengetik...");
    } else {
...
void drawOutputCardOnly() {
    ...
    tft.setFont(&CutePixel24pt); tft.setTextSize(1);
    tft.setTextColor(C_LGRAY);
    tft.setCursor(258, 226); tft.print("RESPONS SISTEM");
    ...
    if (currentMode == 3) {
        ...
        tft.setFont(&CutePixel24pt); tft.setTextSize(1);
        tft.setTextColor(C_DGRAY);
        tft.setCursor(258, 302); tft.print("Hasil kalkulasi");
    } else if (currentMode == 2) {
        String val = lastSpokenWord.length() ? lastSpokenWord : "-";
        drawCenteredText(val, 250, 224, 220, 48, C_SUCCESS, true);
        tft.setFont(&CutePixel24pt); tft.setTextSize(1);
        tft.setTextColor(C_DGRAY);
        tft.setCursor(258, 302); tft.print("Kata terakhir diucapkan");
    } else {
        String val = String(lastChar);
        drawCenteredText(val, 250, 224, 220, 48, C_ACCENT, true);
        tft.setFont(&CutePixel24pt); tft.setTextSize(1);
        tft.setTextColor(C_DGRAY);
        tft.setCursor(258, 302);
...
void drawBrailleDots() {
    ...
    for (int i=0;i<6;i++) {
        ...
        tft.setFont(&CutePixel48pt); tft.setTextSize(1);
...
void drawWiFiScreen() {
    ...
    tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_WHITE,C_PRIMARY);
    tft.setCursor(100,36); tft.print("Pengaturan");
    
    if (wifiSubState == W_MENU) {
        uint16_t stBg=wOK?0x0320:0x3000;
        tft.fillRoundRect(8,52,464,42,6,stBg);
        tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_WHITE,stBg);
        tft.setCursor(16,72);
        if (wOK) {
            tft.print("WiFi: Terhubung ke "); tft.print(WIFI_SSID);
            tft.setCursor(16,86); tft.print("IP: "); tft.print(WiFi.localIP().toString());
        } else {
        ...
        tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_WHITE, C_BG);
        tft.setCursor(120, 174); tft.print("Mencari WiFi...");
    }
        ...
        if (wifiFoundCount == 0) {
            tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_WHITE, C_BG);
            tft.setCursor(16, 92); tft.print("Tidak ada WiFi ditemukan.");
        }
        for (int i=0; i<count; i++) {
            ...
            tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_WHITE, C_SURFACE);
            tft.setCursor(16, y+20); tft.print(ssid);
            tft.setTextColor(C_LGRAY, C_SURFACE);
            tft.setCursor(16, y+36); tft.print(encStr);
        }
        ...
        if (kbV) {
            // compact mode
            tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_LGRAY,C_BG);
            tft.setCursor(8,64); tft.print("SSID: "); tft.print(inputSSID);
            if (inputIsEnt) {
                tft.setCursor(8,80); tft.print("User:");
                drawField(48,60,424,28,inputUser,editingField==FIELD_USER,"Username...");
                tft.setCursor(8,110); tft.print("Pass:");
                ...
                tft.setCursor(8,92); tft.print("Password:");
            ...
            // normal mode
            tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_WHITE, C_BG);
            tft.setCursor(10, 72); tft.print("Menghubungkan ke: "); tft.print(inputSSID);
            
            if (inputIsEnt) {
                tft.setTextColor(C_ACCENT, C_BG);
                tft.setCursor(10, 92); tft.print("Jaringan ini butuh Username (Enterprise)");
                tft.setTextColor(C_LGRAY, C_BG);
                tft.setCursor(10, 116); tft.print("Username:");
                drawField(8, 114, 464, 40, inputUser, false, "Sentuh untuk isi username...");
                tft.setCursor(10, 174); tft.print("Password:");
                ...
                tft.setTextColor(C_LGRAY, C_BG);
                tft.setCursor(10, 112); tft.print("Password:");
...
void showConnectingOverlay(const String &ssid) {
    ...
    tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_LGRAY,C_SURFACE);
    tft.setCursor(100,128); tft.print("Menghubungkan ke:");
    tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_WHITE,C_SURFACE);
    tft.setCursor(100,160); tft.print(ssid.length()>20?ssid.substring(0,20):ssid);
    tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_ACCENT,C_SURFACE);
    tft.setCursor(100,174); tft.print("Harap tunggu (maks 20 detik)...");
...
void showConnectResult(bool ok, const String &ssid) {
    ...
    tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(col,C_SURFACE);
    tft.setCursor(100,140); tft.print(ok?"Berhasil!":"Gagal!");
    tft.setFont(&CutePixel24pt); tft.setTextSize(1); tft.setTextColor(C_LGRAY,C_SURFACE);
    tft.setCursor(100,158);
    if (ok) {
        tft.print("Terhubung: "); tft.print(ssid);
        tft.setCursor(100,172); tft.print("IP: "); tft.print(WiFi.localIP().toString());
        tft.setCursor(100,186); tft.print("Tersimpan ke EEPROM.");
    } else {
        tft.print("Tidak bisa terhubung.");
        tft.setCursor(100,172); tft.print("Periksa SSID dan Password.");
    }
    delay(3000);
}
